...
pub fn read_settings() -> Settings {
    ...
    std::fs::create_dir_all(&home_dir).expect("Failed to create directories to settings.toml path!");
    ...
    file.read_to_string(&mut file_content).expect("Failed to read contents of settings.toml file!");

    return toml::from_str(&file_content).expect("Failed to deserialize content from settings.toml file!");
}

pub fn write_settings(settings: &Settings) {
    let settings_as_str = toml::to_string(&settings).expect("Failed to serialize settings data to string!");
